...
public class RequisicaoDAO implements InterfaceDataBase {
	 ...
	 *            ,P_IN_OUT_REQUISICAO_SQ IN OUT NUMBER ,P_IN_COD_UNIDADE IN
	 *            VARCHAR2 ,P_IN_COD_MA IN VARCHAR2 ,P_IN_COD_SMA IN VARCHAR2
	 *            ,P_IN_USUARIO_SQ IN NUMBER ,P_IN_CARGO_SQ IN NUMBER ,P_IN_COTA
	 *            IN NUMBER ,P_IN_TP_CONTRATACAO IN VARCHAR2 ,P_IN_NM_SUPERIOR
	 *            IN VARCHAR2 ,P_IN_FONE_UNIDADE IN VARCHAR2
	 *            ,P_IN_JORNADA_TRABALHO IN NUMBER ,P_IN_LOCAL_TRABALHO IN
	 *            VARCHAR2 ,P_IN_MOTIVO_SOLICITACAO IN VARCHAR2 ,P_IN_OBS IN
	 *            VARCHAR2 ,P_IN_SUPERVISAO IN VARCHAR2 ,P_IN_NR_FUNCIONARIO IN
	 *            NUMBER ,P_IN_DS_TAREFA IN VARCHAR2 ,P_IN_VIAGEM IN VARCHAR2
	 *            ,P_IN_SALARIO IN NUMBER ,P_IN_OUTRO_LOCAL IN VARCHAR2
	 *            ,P_IN_NM_INDICADO IN VARCHAR2 ,P_IN_INICIO_CONTRATACAO IN DATE
	 *            ,P_IN_FIM_CONTRATACAO IN DATE ,P_IN_COD_RECRUTAMENTO IN NUMBER
	 *            ,P_IN_COD_AREA IN NUMBER ,P_IN_RAZAO_SUBSTITUICAO IN VARCHAR2
	 *            ,P_IN_TIPO_INDICACAO IN VARCHAR2 ,P_IN_DS_MOTIVO_SOLICITACAO
	 *            IN VARCHAR2 ,P_IN_CLASSIFICACAO_FUNCIONAL IN NUMBER
	 *            ,P_IN_ID_INDICADO IN NUMBER ,P_IN_SUBSTITUIDO_ID_HIST IN
	 *            NUMBER ,P_IN_TRANSFERENCIA_DATA IN DATE ,P_IN_IND_CARTA_CONVTE
	 *            IN CHAR ,P_IN_IND_EX_CARTA_CONVTE IN CHAR
	 *            ,P_IN_IND_EX_FUNCIONARIO IN CHAR ,P_IN_IND_TIPO_REQUISICAO IN
	 *            VARCHAR2 ,P_IN_COD_STATUS IN NUMBER ,P_IN_USUARIO IN VARCHAR2
	 *            ,P_IN_SEGMENTO_1 IN VARCHAR2 ,P_IN_SEGMENTO_2 IN VARCHAR2
	 *            ,P_IN_SEGMENTO_3 IN VARCHAR2 ,P_IN_SEGMENTO_4 IN VARCHAR2
	 *            ,P_IN_SEGMENTO_5 IN VARCHAR2 ,P_IN_SEGMENTO_6 IN VARCHAR2
	 *            ,P_IN_SEGMENTO_7 IN VARCHAR2 ,P_IN_NIVEL IN NUMBER
	 *            ,P_IN_DSC_RECRUTAMENTO IN VARCHAR2 ,P_IN_CARATER_EXCECAO IN
	 *            VARCHAR2 ,P_IN_VERSAO_SISTEMA IN VARCHAR2
	...
	private int dmlRequisicao(int tipoDML, Requisicao requisicao, Usuario usuario) throws RequisicaoPessoalException {
		...
		try {
			...
			throw new RequisicaoPessoalException(
					"Ocorreu um erro ao " + tipoTransacao + " a Requisicao: \n" + parametros.toString() + "\n",
					e.getMessage());
		} catch (Exception e) {
			sucesso = 0;
			throw new RequisicaoPessoalException(
	...
	/**
	 * @param tipoRecrutamento
	 *            | 1 - Interno / 2 - Externo / 3 - Misto
	 * @return String[][]
	 * @throws RequisicaoPessoalException
	 */
	...
	public String[][] getComboRecrutamento() throws RequisicaoPessoalException {
		...
		// -- Query que retorna os tipos de recrutamentos de acordo com o tipo
		// de requisição
	...
	/**
	 * @param tipo
	 *            = C - codUnidade | S - siglaUnidade | null - codigo e
	 *            descricao
	 * @return String[][]
	 * @throws RequisicaoPessoalException
	 */
	public String[][] getComboUnidade(String tipo) throws RequisicaoPessoalException {
		...
		// -- Query que retorna informações da unidade de acordo com o parâmetro
		// recebido
		sql.append(" SELECT ");
	 ...
	 * @throws RequisicaoPessoalException
	 */
	public String[][] getComboUnidadesRelacionadas(int codUnidade, String listaUnidades)
			throws RequisicaoPessoalException {
		String complemento = "";
		if (listaUnidades != null && !listaUnidades.equals("") && !listaUnidades.equals(",")
				&& !listaUnidades.equals("0")) {
			complemento = " OR U.COD_UNIDADE IN (" + listaUnidades + ") ";
		}
		return getComboUnidadesRelacionadas(
				" AND (SUBSTR(UO.CODIGO,1,3) = LPAD(" + codUnidade + ",3,0) " + complemento + ") ");
	}
	...
	public String[][] getRequisicoesParaExclusao(String condicao) throws RequisicaoPessoalException {
		...
		sql.append(" AND    R.COD_STATUS BETWEEN 1 AND 5 "); // -- ABERTA, EM
																// HOMOLOGAÇÃO,
																// EM REVISÃO,
																// APROVADA,
																// REPROVADA
	 ...
	 * @return String[][]
	 * @throws RequisicaoPessoalException
	 * @coments Histórico dos campos alterados da requisição informada
	 */
	public String[][] getHistoricoPerfilCampos(int codRequisicao, String dataSource) throws RequisicaoPessoalException {
		StringBuffer sql = new StringBuffer();
		String[][] retorno = null;

		// -- Query que retorna o histórico dos campos alterados da requisição
		// informada
		...
		if (dataSource.equals(InterfaceDataBase.DATA_BASE_NAME_VS_ANTERIOR)) {
			...
			sql.append(" FROM   reqpes.HISTORICO_PERFIL_CAMPOS T ");
		}
	...
	public int getDadosUsuarioCriador(int requisicao_sq) throws RequisicaoPessoalException {
		...
		sql.append(" SELECT u.identificacao                                    		 ");
		sql.append("   FROM reqpes.usuario u                                         ");
		sql.append("  WHERE u.usuario_sq = (SELECT r.usuario_sq                      ");
		sql.append("                          FROM historico_requisicao r            ");
		sql.append("                         WHERE r.requisicao_sq = " + requisicao_sq);
		sql.append("                           AND r.status = 'criou')               ");
		...
		return Integer.valueOf(retorno[0][0]);
	}
	...
	/**
	 * @param datInicio
	 *            (inicio contratação), prazo (prazo de contratação)
	 * @return String
	 * @throws RequisicaoPessoalException
	...
	public String getDataFimContratacao(String datInicio, int prazo) throws RequisicaoPessoalException {
		...
		// sql.append(" SELECT TO_CHAR(TO_DATE('" + datInicio + "',
		// 'dd/mm/yyyy') + 5 + " + prazo + ", 'dd/mm/yyyy') AS FIM_CONTRATACAO
		// ");
		// sql.append(" FROM DUAL ");

		sql.append(" SELECT TO_CHAR(ADD_MONTHS(TO_DATE('" + datInicio + "', 'dd/mm/yyyy'), " + prazo
				+ " ) , 'dd/mm/yyyy') AS FIM_CONTRATACAO ");
		sql.append(" FROM   DUAL ");
