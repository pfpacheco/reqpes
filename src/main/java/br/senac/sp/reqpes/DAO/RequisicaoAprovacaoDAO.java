...
public class RequisicaoAprovacaoDAO implements InterfaceDataBase {
	 ...
	 *            ,P_IN_REQUISICAO_SQ IN NUMBER ,P_IN_COD_UO_APROVADORA IN
	 *            VARCHAR2 ,P_IN_COD_UO_HOMOLOGADOR IN VARCHAR2 ,P_IN_NIVEL IN
	 *            NUMBER ,P_IN_DSC_MOTIVO IN VARCHAR2 ,P_IN_USUARIO IN VARCHAR2)
	 *            IS
	 */
	...
	public int homologaRequisicao(RequisicaoAprovacao requisicao, Usuario usuario) throws RequisicaoPessoalException {
		...
		try {
			...
			stmt.setString(1, "A"); // A indica Aprovação na Homologação do
									// gerente de unidade ou homologador da GEP
	...
	public int aprovaRequisicao(RequisicaoAprovacao requisicao, Usuario usuario) throws RequisicaoPessoalException {
		...
		try {
			stmt = transacao.getCallableStatement("{call reqpes.SP_DML_REQUISICAO_HOMOLOGACAO(?,?,?,?,?,?,?)}");
			stmt.setString(1, "AF"); // AF indica Aprovação Final na Homologação
										// do gerente da Unidade Aprovadora
			stmt.setInt(2, requisicao.getCodRequisicao());
			stmt.setString(3, requisicao.getCodUnidadeAprovadora());
			stmt.setString(4, requisicao.getCodUnidadeHomologador());
			stmt.setInt(5, requisicao.getNivelWorkFlow());
			...
			sucesso = 0;
			throw new RequisicaoPessoalException(
					"Ocorreu um erro em aprovaRequisicao: \nRequisição: " + requisicao.getCodRequisicao() + "\n",
					e.getMessage());
		} catch (Exception e) {
			sucesso = 0;
			throw new RequisicaoPessoalException(
	 ...
	 * @throws RequisicaoPessoalException
	 */
	public String[][] getRequisicoesParaHomologacaoUO(int codUnidade, String codUnidadesLista)
			throws RequisicaoPessoalException {
		// Verifica se houve unidade relacionada ao perfil do usuário
		String complemento = "";
		if (codUnidadesLista != null && !codUnidadesLista.equals("") && !codUnidadesLista.equals(",")
				&& !codUnidadesLista.equals("0")) {
			complemento = " OR V.COD_UNIDADE_NUM IN (" + codUnidadesLista + ") ";
		}
		String[][] requisicao = getRequisicoesParaHomologacao(
				" AND (V.NIVEL_WORKFLOW = 1 OR V.COD_STATUS = 3) AND (V.COD_UNIDADE_DESTINO LIKE LPAD(" + codUnidade
						+ ",3,'0') || '%'  " + complemento + ") ");
	...
	public String[][] getRequisicoesParaHomologacaoGEP() throws RequisicaoPessoalException {
		// ORIGINAL String[][] requisicao = getRequisicoesParaHomologacao(" AND
		// (V.COD_STATUS = 2 AND V.NIVEL_WORKFLOW = 2) OR (V.COD_STATUS = 3 AND
		// V.COD_UNIDADE IN ('002C','012C'))");
		String[][] requisicao = getRequisicoesParaHomologacao(
				" AND (V.COD_STATUS = 2 AND V.NIVEL_WORKFLOW = 3) OR (V.COD_STATUS = 3 AND V.COD_UNIDADE IN ('002C','012C'))");
		return (requisicao.length > 0) ? requisicao : null;
	}
	...
	public String[][] getRequisicoesParaHomologacaoNEC(String listaUO) throws RequisicaoPessoalException {
		// ORIGINAL String[][] requisicao = getRequisicoesParaHomologacao(" AND
		// V.COD_UNIDADE IN ("+ listaUO +") AND ((V.COD_STATUS = 2 AND
		// V.NIVEL_WORKFLOW = 3) OR (V.COD_STATUS = 3 AND V.COD_UNIDADE IN
		// ('002C','012C')))");
		String[][] requisicao = getRequisicoesParaHomologacao(" AND V.COD_UNIDADE IN (" + listaUO
				+ ") AND ((V.COD_STATUS = 2 AND V.NIVEL_WORKFLOW = 2) OR (V.COD_STATUS = 3 AND V.COD_UNIDADE IN ('002C','012C')))");
