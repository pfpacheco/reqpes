...
public class InstrucaoDAO implements InterfaceDataBase {
	 ...
	 *            ,P_IN_COD_INSTRUCAO IN OUT NUMBER ,P_IN_COD_TAB_SALARIAL IN
	 *            NUMBER ,P_IN_COD_CARGO IN NUMBER ,P_IN_COTA IN NUMBER
	 *            ,P_IN_COD_AREA_SUBAREA IN VARCHAR2 ,P_IN_USUARIO IN VARCHAR2)
	 *            IS
	 */
	...
	public String[][] getComboCargo(int codTabelaSalarial) throws RequisicaoPessoalException {
		...
		// -- Carrega a lista de cargos associados das divisões da Tebela
		// Salarial GERAL
		// -- 1 TABELA: 01 GERAL
		...
		if (codTabelaSalarial >= 1 && codTabelaSalarial <= 4) {
			if (codTabelaSalarial == 1) {
				sql.append(
						" AND  NOT EXISTS (SELECT 1 FROM reqpes.TABELA_SALARIAL_CARGOS TC WHERE TC.COD_CARGO = C.ID AND TC.COD_CARGO <> 8473 AND TC.COD_TAB_SALARIAL IN (2,3,4) ) ");
			} else {
				sql.append(
						" AND  EXISTS (SELECT 1 FROM reqpes.TABELA_SALARIAL_CARGOS TC WHERE TC.COD_CARGO = C.ID AND TC.COD_TAB_SALARIAL = "
								+ codTabelaSalarial + ") ");
			}
		}
	...
	public String[] isGravaInstrucao(Instrucao instrucao, int dml, String[] unidades)
		...
		try {
			...
			// -- Caso os dados da instrução estiverem válidos, verifica as
			// unidades na atribuicao
	 ...
	 * @throws RequisicaoPessoalException
	 */
	public int validaIN15(String codUnidade, int codCargo, int cotaCargo, String condicao)
			throws RequisicaoPessoalException {
		StringBuffer sql = new StringBuffer();
		String[][] retorno = null;
