...
public class ResponsavelEstruturaDAO implements InterfaceDataBase {
	 ...
	 *            ,P_UO_CERTA IN OUT VARCHAR2 ,P_ID_GER IN OUT NUMBER
	 *            ,P_TENTATIVA IN OUT NUMBER ,P_TEOR_COD IN OUT VARCHAR2) IS
	 */
	...
	public String[] buscarUnidadeDestino(String codUnidade) throws RequisicaoPessoalException {
		...
		try {
			...
			stmt.registerOutParameter(2, Types.VARCHAR); // retornando a UO
															// correta no
															// WorkFlow
			stmt.registerOutParameter(3, Types.INTEGER); // retornando a chapa
															// do Gerente
															// responsável pela
															// UO
			...
			try {
				...
				throw new RequisicaoPessoalException("Erro ao fechar conexao com ResponsavelEstrutura ",
						e.getMessage());
			}
		}
		return sucesso;
	...
	public boolean isUsuarioWorkflow(int chapa) throws RequisicaoPessoalException, AdmTIException {
		...
		// -- Query que retorna se a chapa informada está no WorkFlow
		...
		try {
			resultado = manipulaDAO.getLista(sql.toString(), DATA_BASE_NAME);
			...
			throw new RequisicaoPessoalException(
					"ResponsavelEstruturaDAO  \n -> Problemas na consulta de isUsuarioWorkflow: \n\n " + sql.toString()
							+ "\n\n erro exibido:" + e.getMessage() + "\n\n qtde:" + resultado.length,
					e.getMessage());
		}
		return (retorno);
	}
	...
	public boolean isAprovadorFinal(int chapa) throws RequisicaoPessoalException, AdmTIException {
		...
		SistemaParametro unidadeAprovadora = sistemaParametroControl
	...
	public boolean isUsuarioHomologadorGEP(int chapa) throws RequisicaoPessoalException {
		...
		try {
			// -- Resgatando os homologadores da GEP
			SistemaParametro[] homologadoresGEP = sistemaParametroControl
					.getSistemaParametrosPorSistemaNome(Config.ID_SISTEMA, "HOMOLOGADOR_GEP");

			// -- Verificando se a chapa informada está na lista de
			// homologadores
	...
	public ResponsavelEstrutura getResponsavelEstrutura(Usuario usuario) throws RequisicaoPessoalException {
		...
		try {
			...
			SistemaParametro idPerfilAPR = sistemaParametroControl.getSistemaParametroPorSistemaNome(Config.ID_SISTEMA,
					"ID_PERFIL_APR_GEP");
			SistemaParametro idPerfilHOM = sistemaParametroControl.getSistemaParametroPorSistemaNome(Config.ID_SISTEMA,
					"ID_PERFIL_HOM_UO");
			SistemaParametro idPerfilGEP = sistemaParametroControl.getSistemaParametroPorSistemaNome(Config.ID_SISTEMA,
					"ID_PERFIL_HOM_GEP");
			SistemaParametro idPerfilNEC = sistemaParametroControl.getSistemaParametroPorSistemaNome(Config.ID_SISTEMA,
					"ID_PERFIL_HOM_NEC");
			...
			if (this.isUsuarioWorkflow(chapa)) {
				if (this.isAprovadorFinal(chapa)) {
					...
					// seta as unidades de acesso do gerente com a da
					// RESPOSAVEL_ESTRUTURA
				...
				if (this.isUsuarioHomologadorNEC(chapa)) {
					...
					// retorna o mesmo perfil, no caso de CRIAÇÃO ou
					// ADMINISTRADOR
	...
	public String getTeorCodWorkflow() throws AdmTIException {
		...
		try {
			SistemaParametro teorCod = sistemaParametroControl.getSistemaParametroPorSistemaNome(Config.ID_SISTEMA,
					"TEOR_COD_WORKFLOW");
	...
	public Unidade[] getUnidadesAcessoWorkflow(Usuario usuario) throws RequisicaoPessoalException, AdmTIException {
		...
		sql.append(
				" SELECT DECODE(SUBSTR(RE.UNOR_COD,0,1), '0', SUBSTR(RE.UNOR_COD,2,2), SUBSTR(RE.UNOR_COD,1,3)) UO, ' '");
		sql.append(" FROM   RESPONSAVEL_ESTRUTURA RE ");
		sql.append(" WHERE  RE.TEOR_COD = '" + this.getTeorCodWorkflow() + "' ");
		...
		try {
			...
			if (!listaUnidadesUsuario.equals("") && !listaUnidadesUsuario.equals(",")
					&& !listaUnidadesUsuario.equals(" ,")) {
				where = listaUnidadesUsuario;
			}

			if (!listaUnidadesWorkflow.equals("") && !listaUnidadesWorkflow.equals(",")
					&& !listaUnidadesWorkflow.equals(" ,")) {
				if (where.equals("")) {
			...
			throw new RequisicaoPessoalException(
					"ResponsavelEstruturaDAO  \n -> Problemas na consulta de getCodUnidadeByUsuarioWorkFlow1: \nCláusula de pesquisa:"
