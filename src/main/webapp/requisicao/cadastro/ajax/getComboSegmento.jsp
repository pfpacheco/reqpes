...
<%
  ...
  if(codSegmento != 0 && codSegmento == 1){
    ...
    sqlQuery.append(" FROM   reqpes.CODE_COMBINATION_RH CC ");
    sqlQuery.append("       ,reqpes.CODE_DESCRICOES_RH  CD ");
    sqlQuery.append(" WHERE  CC.COD_SEGMENTO1 = CD.COD_SEGMENTO ");	     
    sqlQuery.append(" AND    CC.COD_SEGMENTO1 = '1' ");
    
    //-- setando a ação do combo
    acao = "onChange=\"javaScript:carregaCombo(document.frmRequisicao,this.value,2,'divSegmento2',-1,'segmento2', '', '', '', '', '', '"+request.getContextPath()+"/requisicao/cadastro/ajax/getComboSegmento.jsp');\"";
  }

  //--  SEGMENTO 2 (UO Origem)
  if(codSegmento != 0 && codSegmento == 2){
    ...
    //-- setando a ação do combo
    acao = "onChange=\"javaScript:carregaCombo(document.frmRequisicao,this.value,3,'divSegmento3',-1,'segmento3', '', '', '', '', '', '"+request.getContextPath()+"/requisicao/cadastro/ajax/getComboSegmento.jsp');\"";
  }  

  //--  SEGMENTO 3 (UO Destino)
  if(codSegmento != 0 && codSegmento == 3){
    ...
    sqlQuery.append("       ,UNIDADES_ORGANIZACIONAIS UO ");
    sqlQuery.append(" WHERE  CC.COD_SEGMENTO3 = CD.COD_SEGMENTO ");	     
    //sqlQuery.append(" AND    CC.COD_SEGMENTO3 = LPAD(UO.COD_UNIDADE,3,'0') ");
    sqlQuery.append(" AND    CC.COD_SEGMENTO3 = SUBSTR(UO.CODIGO,0,3) ");    
    sqlQuery.append(" AND    CC.COD_SEGMENTO1 = '1' ");
    sqlQuery.append(" AND    CC.COD_SEGMENTO2 = '012' ");
    sqlQuery.append(" AND    CD.TIPO_SEGMENTO = 3 ");
    sqlQuery.append(" AND    UO.DATA_ENCERRAMENTO IS NULL ");
	sqlQuery.append(" AND    NIVEL = 2 ");
	sqlQuery.append(" AND    UO.CODIGO NOT IN ('SA', 'SD', 'SU', 'SO') ");
    ...
    if(codPerfilUsuario == Integer.parseInt(idPerfilHOM.getVlrSistemaParametro()) || (codPerfilUsuario == Integer.parseInt(idPerfilCRI.getVlrSistemaParametro()) && !codUnidadeAPR.getVlrSistemaParametro().equals(codUnidadeRHEV))){
      ...
      sqlQuery.append(" AND (TO_NUMBER(CC.COD_SEGMENTO3) = "+ codUnidadeUsuario +" OR TO_NUMBER(CC.COD_SEGMENTO3) IN ("+ todasUnidades +"))");      
    }
    sqlQuery.append(" ORDER BY CC.COD_SEGMENTO3 ");
    
    //-- setando a ação do combo
    acao = "onChange=\"javaScript:carregaCombo(document.frmRequisicao,this.value,4,'divSegmento4',-1,'segmento4', this.value, '', '', '', '', '"+request.getContextPath()+"/requisicao/cadastro/ajax/getComboSegmento.jsp'); limpaCargoConfig();\"";
  }

  //--  SEGMENTO 4 (Área / Sub-área)
  if(codSegmento != 0 && codSegmento == 4){
    ...
    //-- setando a ação do combo
    acao = "onChange=\"javaScript:carregaCombo(document.frmRequisicao,this.value,5,'divSegmento5',-1,'segmento5', '" + codSegmento3 + "', this.value, '', '', '', '"+request.getContextPath()+"/requisicao/cadastro/ajax/getComboSegmento.jsp');\"";
  	//-- Thiago 18/04/2011: a tabela de monitores não necessita mais de área-subárea
    //-- retirada chamada da função deste combo limpaCargoConfig();
  }  
  
  //--  SEGMENTO 5 (Serviço / Produto)
  if(codSegmento != 0 && codSegmento == 5){
    ...
    //-- setando a ação do combo
    acao = "onChange=\"javaScript:carregaCombo(document.frmRequisicao,this.value,6,'divSegmento6',-1,'segmento6', '" + codSegmento3 + "', '" + codSegmento4 + "', this.value, '', '', '"+request.getContextPath()+"/requisicao/cadastro/ajax/getComboSegmento.jsp');\"";    
  } 
  
  //--  SEGMENTO 6 (Especificação)
  if(codSegmento != 0 && codSegmento == 6){
    ...
    //-- setando a ação do combo
    acao = "onChange=\"javaScript:carregaCombo(document.frmRequisicao,this.value,7,'divSegmento7',-1,'segmento7', '" + codSegmento3 + "', '" + codSegmento4 + "', '" + codSegmento5 + "', this.value, '', '"+request.getContextPath()+"/requisicao/cadastro/ajax/getComboSegmento.jsp');\"";    
  }  
  
  //--  SEGMENTO 7 (Modalidade)
...
<%if(codSegmento != 0){
  segmentosCombo = requisicaoControl.getMatriz(sqlQuery.toString()); 
%>
