...
<jsp:useBean id="requisicaoJornada"
	class="br.senac.sp.reqpes.model.RequisicaoJornada" />
<jsp:setProperty name="requisicaoJornada" property="*" />

<jsp:useBean id="requisicaoPerfil"
	class="br.senac.sp.reqpes.model.RequisicaoPerfil" />
<jsp:setProperty name="requisicaoPerfil" property="*" />

<jsp:useBean id="requisicaoRevisao"
	class="br.senac.sp.reqpes.model.RequisicaoRevisao" />
...
<%
	...
	int codRequisicao = (request.getParameter("codRequisicao") == null)
			? 0
			: Integer.parseInt(request.getParameter("codRequisicao"));
	int chapaGerente = (request.getParameter("chapaGerente") == null)
			? 0
			: Integer.parseInt(request.getParameter("chapaGerente"));
	String indTipoHorarioSel = (request.getParameter("indTipoHorarioSel") == null)
			? "E"
			: request.getParameter("indTipoHorarioSel");
	...
	requisicao.setNumFuncionariosSupervisao(request.getParameter("numFuncionariosSupervisao") == null
			? 0
			: Integer.parseInt(request.getParameter("numFuncionariosSupervisao")));
	...
	requisicao.setCodArea(
			request.getParameter("codArea") == null ? 0 : Integer.parseInt(request.getParameter("codArea")));
	...
	requisicao.setCodClassificacaoFuncional(request.getParameter("codClassificacaoFuncional") == null
			? 0
			: Integer.parseInt(request.getParameter("codClassificacaoFuncional")));
	requisicao.setIdSubstitutoHist(request.getParameter("idSubstitutoHist") == null
			|| request.getParameter("idSubstitutoHist").equals("")
					? 0
	...
	int idPerfilHOM = Integer.parseInt(
			(sistemaParametroControl.getSistemaParametroPorSistemaNome(Config.ID_SISTEMA, "ID_PERFIL_HOM_UO")
	...
	if (usuario.getSistemaPerfil().getCodSistemaPerfil() == idPerfilHOM) {
		...
		isPerfilHOM = true;
	} else if (usuario.getSistemaPerfil().getCodSistemaPerfil() == idPerfilGEP) {
		// Resgata o indicador de envio de e-mails caso o usuário seja HOMOLOGADOR AP&B
		isPerfilGEP = true;
	} else if (usuario.getSistemaPerfil().getCodSistemaPerfil() == idPerfilNEC) {
	...
	if (requisicao.getCodRecrutamento() == 1) {
		...
		requisicao.setDatInicioContratacao(
				ConverteDate.stringToSqlDate("01/" + request.getParameter("datInicioContratacao")));
		...
		if (!requisicao.getIndTipoContratacao().equals("1")) {
			...
			requisicao.setDatFimContratacao(
					ConverteDate.stringToSqlDate("01/" + request.getParameter("datFimContratacao")));
		}
	}
	...
	if (requisicao.getIndStatus() == 3) {
		...
		if (requisicao.getTipoedicao() == 0) {
			retorno = new RequisicaoRevisaoControl().alteraRequisicaoRevisao(requisicaoRevisao, usuario,
					isPerfilHOM);
		}
		...
		if (indEnviarEmails != null && indEnviarEmails.equals("S")) {
			...
			//seta o tipo de edição novamente 
				...
				if (isPerfilGEP || isPerfilNEC) {
					...
					if (requisicaoAprovacaoControl.getNivelAprovacaoAtual(requisicao.getCodRequisicao()) == 4) {
						...
						if (isPerfilGEP) {
							...
							RequisicaoMensagemControl.enviaMensagemHomologacaoAPeB(usuario, requisicao,
									listaEmails);
						} else {
							//-- Aprovaçao NEC => notifica a AP&B
							RequisicaoMensagemControl.enviaMensagemHomologacaoNEC(usuario, requisicao,
									listaEmails);
						}
					}
					...
					RequisicaoMensagemControl.enviaMensagemRevisaoEfetuada(usuario, requisicao,
							requisicaoAprovacaoControl.getEmailResponsavelUO(requisicao.getCodUnidade()));
				}
			}
		}
	} else {
