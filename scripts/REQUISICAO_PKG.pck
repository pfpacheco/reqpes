...
*/
           ...
           IF P_IN_REQUISICAO.OUTRO_LOCAL != V_OUTRO_LOCAL THEN
            insert into historico_perfil_campos (requisicao_sq, usuario_sq, dt_envio, campo, conteudo_anterior, conteudo_novo) values
                (V_REQUISICAO_SQ, V_SQ_USUARIO, P_DATA_HORA, 'Outro local', V_OUTRO_LOCAL,P_IN_REQUISICAO.OUTRO_LOCAL);
           END IF;   

           IF P_IN_REQUISICAO.NM_INDICADO != V_NM_INDICADO THEN
            insert into historico_perfil_campos (requisicao_sq, usuario_sq, dt_envio, campo, conteudo_anterior, conteudo_novo) values
                (V_REQUISICAO_SQ, V_SQ_USUARIO, P_DATA_HORA, 'Nome indicado', V_NM_INDICADO,P_IN_REQUISICAO.NM_INDICADO);
           END IF;   

           IF P_IN_REQUISICAO.INICIO_CONTRATACAO != V_INICIO_CONTRATACAO THEN
            insert into historico_perfil_campos (requisicao_sq, usuario_sq, dt_envio, campo, conteudo_anterior, conteudo_novo) values
                (V_REQUISICAO_SQ, V_SQ_USUARIO, P_DATA_HORA, 'Início da contratação', V_INICIO_CONTRATACAO,P_IN_REQUISICAO.INICIO_CONTRATACAO);
           END IF;   

           IF P_IN_REQUISICAO.FIM_CONTRATACAO != V_FIM_CONTRATACAO THEN
            insert into historico_perfil_campos (requisicao_sq, usuario_sq, dt_envio, campo, conteudo_anterior, conteudo_novo) values
                (V_REQUISICAO_SQ, V_SQ_USUARIO, P_DATA_HORA, 'Fim da contratação', V_FIM_CONTRATACAO,P_IN_REQUISICAO.FIM_CONTRATACAO);
           END IF;   
          ...
          IF P_IN_REQUISICAO.COD_AREA != V_COD_AREA  THEN
           ...
           IF P_IN_REQUISICAO.IND_EX_FUNCIONARIO != V_IND_EX_FUNCIONARIO THEN
              insert into historico_perfil_campos (requisicao_sq, usuario_sq, dt_envio, campo, conteudo_anterior, conteudo_novo) values
              (V_REQUISICAO_SQ, V_SQ_USUARIO, P_DATA_HORA, 'Ex Funcionário', V_IND_EX_FUNCIONARIO,P_IN_REQUISICAO.IND_EX_FUNCIONARIO);
           END IF;   
      ...
      ELSIF (P_IN_DML > 0) THEN
        ...
        IF P_IN_GRAVA_HISTORICO_CHAPA > 0 THEN
           ...
           IF TRIM(P_IN_REQUISICAO_PERFIL.COMENTARIOS) != V_COMENTARIOS THEN
               ...
               insert into debug values('P_IN_REQUISICAO_PERFIL.COMENTARIOS ' || P_IN_REQUISICAO_PERFIL.COMENTARIOS);
               insert into debug values('V_COMENTARIOS ' || V_COMENTARIOS);
