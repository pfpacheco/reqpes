CREATE OR REPLACE PACKAGE BODY REQPES.INTEGRACAO_PKG IS
        ...
        --===========================================================--
          IF (P_OUT_MSG IS NULL) THEN
            ...
            SELECT FC.ID_FUNCIONARIO
              INTO V_FUNC_COMPL_ID
              FROM RHEV.FUNCIONARIO_COMPLEMENTO FC
             WHERE FC.ID_FUNCIONARIO = P_IN_OUT_REG_FUNCIONARIOS.ID;
            
            IF V_FUNC_COMPL_ID IS NULL THEN
               INSERT INTO RHEV.FUNCIONARIO_COMPLEMENTO(ID_FUNCIONARIO)
               VALUES (P_IN_OUT_REG_FUNCIONARIOS.ID);
            END IF;
  ...
  V_DESTINATARIO VARCHAR2(500):= 'gepcontratacoes@sp.senac.br, gepfolhadepagamentoeencargos@sp.senac.br';
